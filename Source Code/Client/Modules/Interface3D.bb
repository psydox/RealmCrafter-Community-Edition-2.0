...
Function UpdateInterface()
	...
	;[~~~] automatic rotation behind character if mourse button 3 is hit cysis145
	If MouseHit(3) = True
		CamYaw# = EntityYaw#(Me\CollisionEN) + 0
		CamPitch# = 0.0
	EndIf
	...
	If CharInteractVisible
		...
		Else
			...
			;If target gets out of range destory the target window cysis145
			If EntityDistance(AI\EN,cam) > 85 
			DestroyCharInteractionWindow()
			HideEntity(ActorSelectEN)
			PlayerTarget = 0
			EndIf
...
;	EndIf
	...
	If EscapeHit And GY_ButtonDown(BCharStats) = True And GY_WindowActive(WCharStats)
		...
		EscapeHit = False
	EndIf
	If (KeyHit(24) And ChatEntry\Alpha# < 0.5 And First TextInput = Null) Or GY_WindowClosed(WCharStats)
	...
	;		GY_UpdateLabel(AttributeDisplayNumbers(i), Str$(Me\Attributes\Value[i]) + " / " + Str$(Me\Attributes\Maximum[i])) ; removed since the numbers were annoying me and i didn't like them because they sucked 	
		EndIf
	Next
...
Function CreateCharInteractionWindow(AI.ActorInstance)
	...
	Name$ = AI\Name$
	TexInteract = LoadTexture("Data\Textures\GUI\target.png", 4)
	WCharInteract = GY_CreateWindow(" ", 0.34, 0.0, 0.35, 0.1, True, False, False, TexInteract) ; Str$(Name$)

	HealthVal = (Float#(AI\Attributes\Value[HealthStat]) / Float#(AI\Attributes\Maximum[HealthStat])) * 500.0
	ManaVal = (Float#(AI\Attributes\Value[ManaStat]) / Float#(AI\Attributes\Maximum[ManaStat])) * 500.0
	

	SCharInteractName = GY_CreateLabel(WCharInteract, 0.5, 0.05, Str$(Name$), 255, 255, 255, Justify_Centre)
	SCharInteractHealth = GY_CreateProgressBar2(WCharInteract, 0.1, 0.29, 0.8, 0.35, HealthVal, 500, 255, 0, 0)
	;SCharInteractMana = GY_CreateProgressBar(WCharInteract, 0.37, 0.555, 0.6, 0.18, ManaVal, 500, 0, 190, 255)
	LCharInteractLevel = GY_CreateLabel(WCharInteract, 0.5, 0.74, Str$(AI\Level), 255, 255, 255, Justify_Centre)
	; LanguageString$(LS_Level) + " " + AI\Level)
...
Function UpdateCharInteractionWindow()
	...
	;ManaVal = (Float#(AI\Attributes\Value[ManaStat]) / Float#(AI\Attributes\Maximum[ManaStat])) * 500.0
	;GY_UpdateProgressBar( SCharInteractMana, ManaVal )
	;GY_UpdateLabel( LCharInteractFaction, LanguageString$(LS_Faction) + " " + FactionNames$(AI\HomeFaction) )
	GY_UpdateLabel( LCharInteractLevel, Str$(AI\Level))
...
Function CreateInterface()
	...
	For i = 0 To 39
		If AttributeDisplays(i)\Width# > 0.0 And AttributeDisplays(i)\Height# > 0.0
			...
			AttributeDisplays(i)\Component = GY_CreateProgressBar3(0, X# - WX1#, Y#, W#, H#, 50, 100, R, G, B)
	...
	;DISABLING THE COMPASS Cysis145
	; Compass
	...
	If tex <> 0
		...
		If ResolutionType = 1 ; 16:9 ratio
			ScaleMesh ActionEN, 30.0, 15.0, 1.0
			PositionEntity ActionEN, -15.0, 7.55, 10.0
		Else 				  ;4:3 ratio
			ScaleMesh ActionEN, 20.0, 15.0, 1.0
...
;CysisLibs Outline
Function Outline(ActorSelectEN, scale# = 1.04)
	m0 = CopyMesh(ActorSelectEN, ActorSelectEN)
	FlipMesh m0
	EntityFX m0, 1+8
